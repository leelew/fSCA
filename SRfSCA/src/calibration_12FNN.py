...
def main():
    ...
    x_train = np.stack([sd_train, swe_train, ta_train, rs_train, rl_train, ps_train, ppt_train, q_train, \
        np.tile(elv_std, (nt_train,1,1)),
        np.tile(elv, (nt_train,1,1)),
        np.tile(slp, (nt_train,1,1)),
        np.tile(asp, (nt_train,1,1))], axis=-1).reshape(-1, 12)
    ...
    x_test = np.stack([sd_test, swe_test, ta_test, rs_test, rl_test, ps_test, ppt_test, q_test, \
        np.tile(elv_std, (nt_test,1,1)),
        np.tile(elv, (nt_test,1,1)),
        np.tile(slp, (nt_test,1,1)),
        np.tile(asp, (nt_test,1,1))], axis=-1).reshape(-1, 12)
