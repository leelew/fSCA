...
def main(X_train, #(N, feat) 
    ...
    # define train set
    feat_name = ['sd', 'revrho', 'swe', 'ta', 'q', 'revstd']
    ...
    # create tmp file
    tempdir = '/tera05/lilu/fSCA/SRfSCA/out/design_feat/tmp/'+'case_{num_feat}_{loss_exp}_{subset_size}_{verylow_complexity}/'.format(
    ...
    with open(os.path.join(model.tempdir_, 'out.txt') , 'a') as file:
        ...
        idx = np.argwhere(mse=np.nanmin(mse))
        file.write("\nBest Equation: {eq} Best MSE: {mse}".format(eq=model.sympy(idx), mse=mse_save[idx]))
...
if __name__ == '__main__':
    ...
    X_train, y_train = np.load('/tera05/lilu/fSCA/SRfSCA/data/design_feat_sd/0.1/x_train.npy'), np.load('/tera05/lilu/fSCA/SRfSCA/data/design_feat_sd/0.1/y_train.npy')
    X_test, y_test = np.load('/tera05/lilu/fSCA/SRfSCA/data/design_feat_sd/0.1/x_test.npy'), np.load('/tera05/lilu/fSCA/SRfSCA/data/design_feat_sd/0.1/y_test.npy')

    config = argparse.ArgumentParser()
    config.add_argument('--num_feat', type=int, default=6)
